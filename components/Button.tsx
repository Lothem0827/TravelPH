...
const Button: React.FC<ButtonProps> = ({
    ...
    const getVariantStyles = () => {
        switch (variant) {
            ...
            case 'outline':
                return 'bg-white active:bg-yellow-50 border border-yellow-400';
            default:
                return 'bg-yellow-400 active:bg-yellow-500 border-transparent';
        }
    };
    ...
    return (
        <TouchableOpacity
            className={`w-full py-3.5 rounded-full flex-row items-center justify-center border ${getVariantStyles()} ${className}`}
            {...props}
        >
            {icon && <View className="mr-2">{icon}</View>}
            <Text
                className={`text-center font-bold text-yellow-900 ${textClassName}`}
                numberOfLines={1}
